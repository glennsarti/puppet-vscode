...
export class RubyConfiguration implements IRubyConfiguration {
  ...
  get puppetBaseDir(): string {
    ...
    switch (this.config.installType) {
      case settings.PuppetInstallType.agent:
  ...
  // get protocol(): settings.ServiceProtocol {
  //   switch (this.config.editorService.protocol) {
  //     case settings.ServiceProtocol.tcp:
  //       return settings.ServiceProtocol.tcp;
  //     // Default is stdio
  //     default:
  //       return settings.ServiceProtocol.stdio;
  //   }
  // }
  ...
  get languageServerCommandLine(): Array<string> {
    ...
    switch (this.config.editorService.protocol) {
      case settings.ServiceProtocol.tcp:
        if (this.config.editorService.tcp.address === undefined || this.config.editorService.tcp.address === '') {
          args.push('--ip=127.0.0.1');
        } else {
          args.push('--ip=' + this.config.editorService.tcp.address);
        }
        if (this.config.editorService.tcp.port !== 0) {
          args.push('--port=' + this.config.editorService.tcp.port);
        }
        break;
      // Default is stdio
      default:
    ...
    args.push('--timeout=' + this.config.editorService.timeout);

    if (vscode.workspace.workspaceFolders !== undefined) {
      args.push('--local-workspace=' + vscode.workspace.workspaceFolders[0].uri.fsPath);
    }

    if (this.config.editorService.featureflags.indexOf('filecache') !== -1) {
      args.push('--enable-file-cache');
    }

    if (this.config.editorService.debugFilePath !== undefined && this.config.editorService.debugFilePath !== '') {
      args.push('--debug=' + this.config.editorService.debugFilePath);
    }
