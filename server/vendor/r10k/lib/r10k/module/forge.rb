...
class R10K::Module::Forge < R10K::Module::Base
  ...
  def initialize(title, dirname, expected_version, environment=nil)
    ...
    @version_from_puppetfile = expected_version.nil? ? :latest : expected_version
  ...
  def fetch_metadata
    begin
      if version_from_puppetfile == :latest
        v3_module = PuppetForge::Module.find(title)
        v3_module.fetch
        return v3_module.current_release.metadata
      else
        v3_release = PuppetForge::Release.find(title + "-#{version_from_puppetfile}")
        return v3_release.metadata
      end
    rescue Faraday::ResourceNotFound => exception
      # A 404 means the module or release does not exist
      return { :metadata_status => :missing } if exception.response[:status] == 404
