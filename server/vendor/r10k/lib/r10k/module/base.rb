...
class R10K::Module::Base
  ...
  attr_reader :version_from_puppetfile
  ...
  def metadata
    result = R10K::Module.get_metadata_from_cache(cache_key)
    if result.nil?
      begin
        result = fetch_metadata

        if result[:metadata_status].nil?
          result[:metadata_status] = result.empty? ? :unknown : :known
        end
        R10K::Module.set_metadata_in_cache(cache_key, result)
      rescue => exception
        PuppetLanguageServer.log_message(:error, "Error occured getting the metadata for module '#{title}': #{exception}")
        result = {}
      end
    end
    result
