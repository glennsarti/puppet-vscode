...
describe 'message_router' do
    ...
    context 'given a textDocument/completion request' do
      ...
      context 'for a Puppetfile file' do
        ...
        it 'should not log an error message' do
          expect(PuppetLanguageServer).to_not receive(:log_message)
  ...
  describe '#receive_notification' do
    ...
    context 'given a textDocument/didOpen notification' do
      shared_examples_for "an opened document" do |file_uri|
      ...
      context 'for a puppet manifest file' do
        before(:each) do
          expect(PuppetLanguageServer::DocumentValidator).to receive(:validate).and_return([])
          allow(subject).to receive(:reply_diagnostics).and_return(true)
        end
        it_should_behave_like "an opened document", MANIFEST_FILENAME
      end
      ...
      context 'for a Puppetfile file' do
        ...
        it_should_behave_like "an opened document", PUPPETFILE_FILENAME
      end
    end
    ...
    context 'given a textDocument/didChange notification and a TextDocumentSyncKind of Full' do
      shared_examples_for "a changed document" do |file_uri|
        let(:notification_params) { {
          'textDocument' => {
            'uri' => file_uri,
            'version' => 2,
      ...
      context 'for a puppet manifest file' do
        ...
        it_should_behave_like "a changed document", MANIFEST_FILENAME
      end
      ...
      context 'for a Puppetfile file' do
        ...
        it_should_behave_like "a changed document", PUPPETFILE_FILENAME
      end
      ...
      context 'for a file the server does not understand' do
        ...
        it_should_behave_like "a changed document", UNKNOWN_FILENAME
      end
    end
